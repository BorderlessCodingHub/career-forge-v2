...
Last updated: **2026-09-16** · Last merge: **PR #87** [CAR-108](https://linear.app/career-forge-v2/issue/CAR-108) Password mode Borderless sign-in entitles (`2a2547b`) · prior **PR #85** [CAR-101](https://linear.app/career-forge-v2/issue/CAR-101) Borderless password login — CF stays IdP (`10ae705`) · **PR #83** [CAR-100](https://linear.app/career-forge-v2/issue/CAR-100) Pilot enter without OTP · Epic **CAR-28** F3b **complete** · Epic **CAR-22** closed  
...
**Next eng:** **F3a** — [CAR-36](https://linear.app/career-forge-v2/issue/CAR-36) pilots · [CAR-37](https://linear.app/career-forge-v2/issue/CAR-37) pt-BR · [CAR-93](https://linear.app/career-forge-v2/issue/CAR-93) Welcome **Career Forge** pilot quotes (interim Borderless stories shipped in CAR-99). **Ops:** Labs password cutover = [DEPLOY-LABS-MANUAL §2.5](./DEPLOY-LABS-MANUAL.md) (`IDENTITY_METHOD=borderless_password`, `MEMBERSHIP_BACKEND=stub`); recreate backend; members URL **not** required. Repo default stays empty `IDENTITY_METHOD`.

**Deploy:** Auto-deploy on `main` (CAR-13). **CAR-108 Done** — PR #87 password mode entitles via `users.borderless_user_id` (no members HTTP); SSE 402 → `PaywallError`; Labs `MEMBERSHIP_BACKEND=stub`. **CAR-101 Done** — PR #85 Borderless password credential check (CF still IdP): `IDENTITY_METHOD`, `POST /auth/signin`, 410 learner OTP/pilot in password mode, IdentityGate password UI, cutover docs. **CAR-100 Done** — PR #83 learner OTP freeze: Labs `IDENTITY_EMAIL_OTP=false` (recreate backend+frontend); `POST /auth/pilot/enter` + exclusive `billing_pilot_emails`; restore OTP with `true`. **CAR-99 Done** — PR #82 Welcome testimonials are named Borderless BASE/PSP stories (Arthur, Eduardo, Gelson, Thiago); heading “with Borderless”; not Career Forge alumni. **PR #81** — prod compose injects `LANGCHAIN_TRACING_V2` so Labs LangSmith traces work (recreate backend after pull). **PR #80** — `GET /roadmap/current` keeps mixed forge snapshots as one `ai_generated` sequence (forge order + titles; spine no longer splits catalog IDs). **PR #79** — prefer active forge snapshot over leftover generated-only rows. **CAR-98 Done** — PR #77 `rag-embeddings` fundamentals lesson + figure `basePath`; Operator still must set sidecar `published`. **CAR-97 Done** — PR #76 `/learn` additive blocks (`![ ]` figure, `>` callout, fenced `quiz` MCQ). **PR #75** — Testimonials heading without `640+`; External copy is **$15/mo** (not a $10–15 band). **CAR-96 Done** — PR #74 Welcome unmounts Marketing week map (`CurriculumRoadmap` stays in repo; no `#curriculum` hashes). **CAR-95 Done** — PR #72 five RAG git bodies (`rag-embeddings` … `rag-production`); Operator still must set sidecar `published`. **CAR-94 Done** — PR #71 `/learn/{skill_id}` + deterministic attach. **CAR-92 Done** — PR #70 Welcome honesty pass (CAR-91 bar included): BASE·PSP included + External $15/mo copy; no scenery modals; Yuri + Pedro mentors. Testimonials: CAR-99 PR #82 Borderless BASE/PSP stories (Unsplash fakes gone); CAR-93 still for Career Forge pilot quotes. **CAR-90 Done** — PR #68 Operator pending/liberated embed queue from live evidence + `/reference` live allowlist; sandbox `allow-same-origin` + self-origin guard. **CAR-89 Done** — PR #67 `embed_hosts` + Content desk CRUD + learner `GET /reference/embed-hosts`; run Alembic 019. **CAR-88 Done** — PR #66 Access desk list/add/remove for `billing_pilot_emails` + `pilot list` badges. **CAR-87 Done** — PR #65 `billing_pilot_emails` + operator APIs; run Alembic 018 with legacy env populated, then clear `ENTITLEMENT_BILLING_ALLOWLIST`. **CAR-86 Done** — PR #64 Reference source card (allowlist-only iframe). **CAR-80 Done** — PR #63 F3c docs amend. **CAR-85 Done** — PR #62 in-product Reference viewer (`/reference`). **CAR-79 Done** — PR #61 Content sidecar + desk. **CAR-78 Done** — PR #60 Access card UI. **CAR-77 Done** — PR #59 Access desk writes + audit. **CAR-84 Done** — PR #58 Resend `User-Agent` (Cloudflare 1010). **CAR-83 Done** — PR #57 injects `JWT_SECRET`. **CAR-81 Done** — PR #54 injects mailer env; deploy `git pull --ff-only origin main` before `compose up`. VPS `APP_DIR` must be owned by `VPS_USER` (no `sudo git`). Bake `API_INTERNAL_URL` at frontend build (CAR-19). Forge enqueue via `POST /forge/runs` (CAR-20). Trail fetch via `GET /roadmap/current` to avoid App Router page HTML (CAR-30).
...
| [CAR-96](https://linear.app/career-forge-v2/issue/CAR-96) | Welcome Marketing week map unmounted | [P] | **Done** — PR #74 · `CurriculumRoadmap` off Welcome until remount decision |
...
| [CAR-101](https://linear.app/career-forge-v2/issue/CAR-101) | Epic: Borderless password — CF stays IdP | — | **Done** — PR #85 · CAR-102…107 · [ADR-008](./decisions/ADR-008-borderless-password-credential-check.md) |
| [CAR-108](https://linear.app/career-forge-v2/issue/CAR-108) | Password mode: Borderless sign-in entitles | [P] | **Done** — PR #87 · no members HTTP; entitled via `borderless_user_id` |
...
**Start order:** CAR-36 pilots · CAR-37 pt-BR · CAR-93 Career Forge pilot quotes. **CAR-33–35 Done** · **CAR-38 Done** · **CAR-39 Done** · **CAR-40 Done** · **CAR-41 Done** · **CAR-34 Done** · **CAR-54 Done** · **CAR-55 Done** · **CAR-56 Done** · **CAR-57 Done** · **CAR-70 Done** · **CAR-53 Done** · **CAR-91 Done** · **CAR-92 Done** · **CAR-96 Done** · **CAR-99 Done.**
...
| Pilot enter freeze (no OTP) | ✅ Done | CAR-100 — PR #83 · `IDENTITY_EMAIL_OTP=false`; exclusive `billing_pilot_emails`; does not steal existing emails |
| Borderless password (CF IdP) | ✅ Done | CAR-101 — PR #85 · ADR-008; `POST /auth/signin`; IdentityGate password; Labs flip = DEPLOY-LABS-MANUAL §2.5 |
| Password-mode entitlement | ✅ Done | CAR-108 — PR #87 · `borderless_user_id` entitles; no members HTTP; SSE 402 → `PaywallError`; `MEMBERSHIP_BACKEND=stub` |
...
| Canonical skill content (`/learn`) | ✅ Done | CAR-94 — PR #71 attach + `/learn`; CAR-95 — PR #72 five RAG bodies (publish sidecar on desk); CAR-97 — PR #76 figure/callout/quiz parser; CAR-98 — PR #77 embeddings fundamentals body + `learnAssetSrc` |
...
| Welcome honesty pass | ✅ Done | CAR-92 — PR #70 · CAR-91 bar included; BASE·PSP included + External $15/mo; no scenery modals; Yuri + Pedro; CAR-96 — PR #74 Marketing week map unmounted; PR #75 drop `640+` + $15 point price; CAR-99 — PR #82 Borderless BASE/PSP stories (not CF alumni); CAR-93 still for Career Forge pilot quotes |
