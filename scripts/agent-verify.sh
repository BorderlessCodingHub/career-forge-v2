...
check grep -q 'list_pending_embed_hosts' apps/backend/src/career_forge/api/operator.py
...
check test -f apps/frontend/src/components/operator/ContentDesk.tsx
check grep -q 'operator-access-card' apps/frontend/src/components/operator/AccessDesk.tsx
check grep -q 'operator-content-list' apps/frontend/src/components/operator/ContentDesk.tsx
...
check grep -q 'BORDERLESS_SIGNUP_URL' .env.example
...
check test -f docs/engineering/MEMBERSHIP.md
check test -f docs/engineering/ENTITLEMENT.md
...
check grep -q 'COPY --from=builder /app/public ./public' apps/frontend/Dockerfile
