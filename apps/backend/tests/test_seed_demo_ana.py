...
def test_demo_ana_constants() -> None:
    ...
    assert node_ids == {"rag-embeddings", "rag-chunking"}
    assert all(row["passed"] for row in DEMO_ANA_VALIDATIONS)
    assert DEMO_ANA_SKILL_STATE["rag-grounding"]["status"] == "validar"
...
def test_demo_catalog_state_pitch_ready() -> None:
    ...
    http = next(node for node in roadmap.nodes if node.node_id == "rag-retrieval")
    rest = next(node for node in roadmap.nodes if node.node_id == "rag-grounding")
