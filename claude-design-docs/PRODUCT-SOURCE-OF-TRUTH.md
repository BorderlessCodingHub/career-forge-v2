...
| `/roadmap/forge` | **Timeline only** — numbered steps, no graph during stream; research rows may show query + official source cards; brief post-`graph_ready` hold before reveal | SSE wiring, scroll behavior |
...
| Forge research sources | Timeline-only stream with `artifact_found` rows | Mock artifacts without live sources | HAC-54 — OpenAI native `web_search` citations render as source cards, then brief `graph_ready` hold before reveal | **Code + docs aligned** — no third-party search adapter | HAC-54 |
