...
| Artifact roadmap chrome | Centered page `<h1>` + inline mentor card | Topbar track name + actions | **`ArtifactChromeProvider`** in `ArtifactShellLayout`; `/roadmap` sets `trailSummary` via `computeTrailStudySummary`; **`ArtifactShell`** topbar (`artifact-topbar`): right cluster `items-end` — single `mentor-report-link` (`h-9`, `FileText` icon slot); optional `trail-study-summary`; mentor entry = **`NodeDrawer`** `open-mentor-drawer` only (no topbar `mentor-cta`); page intro = subtitle only (`pt-6`); `?adaptive=1` without session → silent server fallback (no warning banner); `VerticalSpineSkeleton` while loading; spine `scrollIntoView` on select; selected spine dot mint glow | **Code wins** — canvas-first artifact steady state | 2026-05-30 |
