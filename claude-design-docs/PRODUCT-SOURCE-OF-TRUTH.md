...
| Node drawer layout | Drawer repeats title + long description | Card shows description; drawer lists status/mastery | **NodeDrawer** — header title only; no duplicate `description` body (read on canvas card); rationale callout uses `text-text-primary` on `bg-surface` | **Code wins** — less redundancy, clearer hierarchy | 2026-05-28 |
