...
| `artifact` | Steady `/roadmap`, `/reference?node=&item=`, adaptive recalibration, **`/report`**, **`/share/[token]`** (read-only list, no shell) | Steady roadmap/`/reference`/`/report`: `ArtifactShell` topbar — logo; right cluster `items-end`; `mentor-report-link` → `/report` + track block; page intro = subtitle + optional `trail-progress-ring`; spine + solid `roadmap-connector-{id}` lines; **no** stepper / progress sidebar |
...
- Premium dev-tool feel — no LMS chrome, no confetti/gamification on product or Welcome UI
...
| `/welcome` Marketing (CAR-56→92) | Premium B chrome in `marketing/welcome/`; Start diagnosis→`/`; BASE/PSP included + External $15/mo copy (no Stripe on Welcome); no apply/strategy/syllabus modals; Borderless employer logos + real mentors; Unsplash testimonials until CAR-93; indexed; Plus Jakarta Sans on shell only | pt-BR CAR-37; CAR-93 quotes |
...
| `/reference?node=&item=` | CAR-85/86 in-product viewer for one Node Reference: Node title + same `done` checkbox + siblings; source card by default; allowlist-only iframe after host evidence; primary source-site escape hatch; CTA restores Node drawer; never raw `?url=` or `/learn` | Add host-specific preview adapters only after successful embed proof |
| `/learn/[skillId]` | CAR-94 in-app Canonical skill content: live sidecar title + git markdown; NodeDrawer link only when focus + published; missing = silence / return to Roadmap | Public blog mirror later |
...
| Node Reference access | Drawer opens external host in a new tab | Prototype has no Reference viewer | **CAR-85/86/89/90** — `/reference?node=&item=` under Email identity; source card default; live learner allowlist GET controls the sandboxed iframe; revoke restores the card without deploy; sibling switcher, same checklist command; return to `/roadmap?node=` | **ADR-007 amendment + code win** — no blank refused frame; opening is not `done`; no backend probe/proxy/ingest; `/learn` remains canonical content | 2026-08-25 |
| Canonical skill content | Not in prototype | Prototype has no `/learn` | **CAR-94** — `/learn/{skill_id}` under Email identity; NodeDrawer link only when focus + published canônico; git markdown + live sidecar title; unpublished = silence | **ADR-004 code win** — N:1, not per-forge posts | 2026-08-26 |
...
| Welcome honesty (CAR-92) | Not in prototype | Vite B fakes | Next `marketing/welcome/` only: bar BASE/PSP included (CAR-91); Hero/SocialProof/Mentors/Pricing/ROI/CTA/Footer/FAQ honest; modals+confetti removed; testimonials Unsplash until CAR-93; $15/mo copy not checkout | **Grill lock CAR-53 + code win (CAR-92)** | 2026-08-25 |
...
| Operator console desks (CAR-76/78/79/88/90) | Not in learner prototype | Keeper stub = desk rooms (CAR-67 variant A) | `/operator` in the same Next app: separate OTP gate; role-filtered Access/Content rooms; Access has cost/pilot/learner grants; Content exposes catalog metadata plus a live Reference host proof queue (pending grouped hosts → sandbox preview + explicit confirmation → liberate; liberated → revoke); no learner identity in queue, ID minting, or markdown editor | **Linear F3c + CAR-90 spec and code win** | 2026-08-25 |
| IdentityGate password (CAR-106) | Not in prototype | Platform sign-in layout (not a clone) | Overlay on product routes: email+password → CF `POST /auth/signin`; product topbar (`BrandLockup` + Back to Welcome) over Welcome-hero backdrop (OTP/pilot share the same shell); signup modal then Borderless `_blank`; no OAuth; no `/sign-in` route | **Grill 2026-09-11 + code win** | 2026-09-12 |
