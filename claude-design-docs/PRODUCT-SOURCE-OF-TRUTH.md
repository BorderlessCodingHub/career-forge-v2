...
| `artifact` | Steady `/roadmap`, adaptive recalibration | `ArtifactShell` topbar: logo; paired `mentor-cta` + `mentor-report-link` (`topbarActionClass`, `FileText` on relatório); track name + optional `trail-study-summary`; page body = subtitle only; **no** stepper; **no** fixed progress sidebar |
...
| `/roadmap` | **Vertical roadmap** steady state; track name in **artifact topbar** only; `mentor-cta` + `mentor-report-link` share `topbarActionClass` (`ArtifactChromeContext` on `/roadmap`); relatório = `FileText` icon + label; optional `trail-study-summary` mint line; canvas compact study progress; drawer accordions + sticky validate CTA | Node detail panel, full AI sidebar (P2) |
...
*Last updated: 2026-05-28 — artifact topbar paired actions (`h-9` + matched `h-7` icon slots)*
