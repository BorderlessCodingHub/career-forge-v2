...
| Node drawer layout | Drawer repeats title + long description; muted `slideover-close` | Card shows description; drawer lists status/mastery | **NodeDrawer** — header title only; no duplicate `description` body (read on canvas card); rationale callout `text-text-primary` on `bg-surface`; header **✕** = red dismiss icon (`h-9 w-9`, `text-red-400`, hover/focus `red-900/60`, `aria-label="Fechar detalhes"`); Escape closes + focus returns to card; collapsible sections (outcomes/refs default closed); compact mentor row; sticky `validate-node-cta` footer | **Code wins** — less redundancy; dismiss reads as exit/destructive, not neutral chrome | 2026-05-28 |
| Artifact roadmap chrome | Centered page `<h1>` + inline mentor card | Topbar track name + actions | **`ArtifactChromeProvider`** in `ArtifactShellLayout`; `/roadmap` sets `onOpenMentor` + `trailSummary` via `computeTrailStudySummary`; **`ArtifactShell`** topbar (`artifact-topbar`): right cluster `items-end` — action pair bottom-aligned to track title (not `items-center`); `mentor-cta` + `mentor-report-link` in inner `flex items-center` group, share `topbarActionClass` (`h-9`); mentor = `MentorAvatar` (`h-7`); relatório = `h-7 w-7` icon slot + `FileText`; `trail-study-summary`; page intro = subtitle only (`pt-6`); `VerticalSpineSkeleton` while loading; spine `scrollIntoView` on select; selected spine dot mint glow | **Code wins** — canvas-first artifact steady state | 2026-05-28 |
