...
| Operator console desks (CAR-76/78/79) | Not in learner prototype | Keeper stub = desk rooms (CAR-67 variant A) | `/operator` in the same Next app: separate OTP gate; role-filtered Access/Content rooms; Access has read-only monthly cost strip, learner grants, Stripe lock, and audit; Content exposes the 40 static catalog IDs with title/URL/published sidecar metadata and read-only git-body status; no ID minting or markdown editor | **Linear F3c spec + code win** | 2026-08-24 |
