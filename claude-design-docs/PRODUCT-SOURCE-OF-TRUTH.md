...
| `artifact` | Steady `/roadmap`, adaptive recalibration | `ArtifactShell` topbar: logo, `mentor-cta`, relatório mentor, track name + optional `trail-study-summary`; page body = subtitle only; **no** stepper; **no** fixed progress sidebar |
...
| `/roadmap` | **Vertical roadmap** steady state; track name in **artifact topbar** only; mentor via `mentor-cta` in topbar (`ArtifactChromeContext` on `/roadmap`); optional `trail-study-summary` mint line; canvas compact study progress; drawer accordions + sticky validate CTA | Node detail panel, full AI sidebar (P2) |
...
*Last updated: 2026-05-28 — artifact roadmap chrome; deploy badge `z-auto` under drawers*
