...
| `/reference?node=&item=` | CAR-85 in-product viewer for one Node Reference: Node title + same `done` checkbox + siblings; sandboxed best-effort iframe; always-visible source-site escape hatch; CTA restores Node drawer; never raw `?url=` or `/learn` | Host-specific preview adapters only after evidence |
...
| Node Reference access | Drawer opens external host in a new tab | Prototype has no Reference viewer | **CAR-85** — `/reference?node=&item=` under Email identity; one sandboxed preview slot, sibling switcher, same checklist command, persistent escape hatch, return to `/roadmap?node=` | **ADR-007 + code win** — opening is not `done`; `/learn` remains canonical content | 2026-08-24 |
