...
| `/welcome/plg` | Product-led (EN); forge HTML mock in hero; before/during/after; dark tokens; not linked from `/welcome` | Visual polish |
...
| `/welcome/plg` | Not in prototype | N/A | EN product-led exploration; default `/welcome` unchanged and unlinked; same CTA → `/`; no price/email/fake proof; HTML/CSS forge mock (`ForgeProductMock`). Editorial + offer explorations discarded. | **Grill + code win** | 2026-08-17 |
