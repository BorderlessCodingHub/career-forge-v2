...
| Node Reference access | Drawer opens external host in a new tab | Prototype has no Reference viewer | **CAR-85/86** — `/reference?node=&item=` under Email identity; source card default (title, hostname, `outcome` or honest fallback, primary Open original); allowlist-only iframe after proven host support; sibling switcher, same checklist command, return to `/roadmap?node=` | **ADR-007 amendment + code win** — no blank refused frame; opening is not `done`; no backend probe/proxy/ingest; `/learn` remains canonical content | 2026-08-24 |
...
| Operator console desks (CAR-76/78/79/88) | Not in learner prototype | Keeper stub = desk rooms (CAR-67 variant A) | `/operator` in the same Next app: separate OTP gate; role-filtered Access/Content rooms; Access has read-only monthly cost strip, pilot billing email list CRUD with explicit `pilot list` badges, learner grants, Stripe lock, and audit; deleting a pilot shortcut never toggles `billing_entitled`; Content exposes the 40 static catalog IDs with title/URL/published sidecar metadata and read-only git-body status; no ID minting or markdown editor | **Linear F3c spec + code win** | 2026-08-25 |
