...
| `artifact` | Steady `/roadmap`, adaptive recalibration | `ArtifactShell` topbar: logo; right cluster `items-end`; `mentor-report-link` + track block; `topbarActionClass`; page intro = subtitle + optional `trail-progress-ring` (`getTrailChecklistProgressPct`); **no** stepper; **no** fixed progress sidebar |
...
| `/roadmap` | **Vertical roadmap** steady state; track name in **artifact topbar** only; right cluster `items-end`; `mentor-report-link` bottom-aligned to track title; page intro (`pt-6`) = subtitle + centered **`trail-progress-ring`** when checklist items exist (`getTrailChecklistProgressPct` item-pooled); canvas compact study progress; drawer accordions + sticky validate CTA | Node detail panel, full AI sidebar (P2) |
...
| Artifact roadmap chrome | Centered page `<h1>` + inline mentor card | Topbar track name + actions | **`ArtifactShell`** topbar (`artifact-topbar`): right cluster `items-end` — single `mentor-report-link` (`h-9`, `FileText` icon slot) + track name block (no ring in topbar); **`TrailProgressRing`** (`trail-progress-ring`, ~44px, mint stroke) centered in **page intro** below subtitle when checklist items exist — `getTrailChecklistProgressPct` (`sum(checklist_completed) / sum(checklist_total)` via `getChecklistProgress`, item-pooled); label **Progresso de estudo**; adaptive view (`?adaptive=1`) = subtitle + highlighted spine node only — **no** `MissionBanner` on canvas; without session → silent server fallback; `VerticalSpineSkeleton` while loading; spine `scrollIntoView` on select; selected spine dot mint glow | **Code wins** — de-cluttered topbar; trail progress lives with canvas intro | 2026-05-30 |
...
*Last updated: 2026-05-30 — `TrailProgressRing` moved from topbar to page intro (canvas de-clutter)*
