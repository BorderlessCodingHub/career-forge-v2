...
| `artifact` | Steady `/roadmap`, adaptive recalibration | `ArtifactShell` topbar: logo; right cluster `items-end`; paired actions + track block; `topbarActionClass`; optional `trail-study-summary`; page body = subtitle only; **no** stepper; **no** fixed progress sidebar |
...
| `/roadmap` | **Vertical roadmap** steady state; track name in **artifact topbar** only; right cluster `items-end` (`ArtifactChromeContext`); actions bottom-aligned to track title; optional `trail-study-summary` mint line; canvas compact study progress; drawer accordions + sticky validate CTA | Node detail panel, full AI sidebar (P2) |
...
| Node drawer layout | Drawer repeats title + long description; muted `slideover-close` | Card shows description; drawer lists status/mastery | **NodeDrawer** — header title only; no duplicate `description` body (read on canvas card); rationale callout `text-text-primary` on `bg-surface`; header **✕** = red dismiss icon (`h-9 w-9`, `text-red-400`, hover/focus `red-900/60`, `aria-label="Fechar detalhes"`); Escape closes + focus returns to card; collapsible sections default open (outcomes, tasks, refs; user can collapse via header toggle); compact mentor row; sticky `validate-node-cta` footer | **Code wins** — less redundancy; dismiss reads as exit/destructive, not neutral chrome | 2026-05-30 |
| Artifact roadmap chrome | Centered page `<h1>` + inline mentor card | Topbar track name + actions | **`ArtifactChromeProvider`** in `ArtifactShellLayout`; `/roadmap` sets `trailSummary` via `computeTrailStudySummary`; **`ArtifactShell`** topbar (`artifact-topbar`): right cluster `items-end` — single `mentor-report-link` (`h-9`, `FileText` icon slot); optional `trail-study-summary`; mentor entry = **`NodeDrawer`** `open-mentor-drawer` only (no topbar `mentor-cta`); page intro = subtitle only (`pt-6`); adaptive view (`?adaptive=1`) = subtitle + highlighted spine node only — **no** `MissionBanner` / `adaptive-mission-banner` on canvas; without session → silent server fallback; `VerticalSpineSkeleton` while loading; spine `scrollIntoView` on select; selected spine dot mint glow | **Code wins** — canvas-first artifact steady state | 2026-05-30 |
...
*Last updated: 2026-05-28 — artifact topbar `items-end` alignment (actions baseline with track title)*
