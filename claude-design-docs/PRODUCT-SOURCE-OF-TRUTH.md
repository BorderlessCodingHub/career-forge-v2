...
| `artifact` | Steady `/roadmap`, adaptive recalibration | `ArtifactShell` topbar: logo; right cluster `items-end`; paired actions + track block + optional `trail-progress-ring`; `topbarActionClass`; page body = subtitle only; **no** stepper; **no** fixed progress sidebar |
...
| `/roadmap` | **Vertical roadmap** steady state; track name in **artifact topbar** only; right cluster `items-end` (`ArtifactChromeContext`); actions bottom-aligned to track title; optional `trail-progress-ring` (`getTrailChecklistProgressPct` — pooled checklist items); canvas compact study progress; drawer accordions + sticky validate CTA | Node detail panel, full AI sidebar (P2) |
...
| Artifact roadmap chrome | Centered page `<h1>` + inline mentor card | Topbar track name + actions | **`ArtifactChromeProvider`** in `ArtifactShellLayout`; `/roadmap` sets `trailProgressPct` via `getTrailChecklistProgressPct` (`sum(checklist_completed) / sum(checklist_total)` per topic via `getChecklistProgress`, item-pooled — not topic-average); **`ArtifactShell`** topbar (`artifact-topbar`): right cluster `items-end` — single `mentor-report-link` (`h-9`, `FileText` icon slot); **`TrailProgressRing`** (`trail-progress-ring`, ~44px, mint stroke) beside track name when checklist items exist; page intro = subtitle only (`pt-6`); adaptive view (`?adaptive=1`) = subtitle + highlighted spine node only — **no** `MissionBanner` on canvas; without session → silent server fallback; `VerticalSpineSkeleton` while loading; spine `scrollIntoView` on select; selected spine dot mint glow | **Code wins** — canvas-first artifact steady state | 2026-05-30 |
...
*Last updated: 2026-05-30 — `getTrailChecklistProgressPct` item-pools per-topic checklist stats for topbar ring*
