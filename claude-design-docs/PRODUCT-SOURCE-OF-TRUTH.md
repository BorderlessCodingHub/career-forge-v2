...
| Operator console + Access card (CAR-76/78) | Not in learner prototype | Keeper stub = desk rooms (CAR-67 variant A) | `/operator` in the same Next app: separate OTP gate; role-filtered Access/Content rooms; Access has read-only monthly cost strip, email lookup, membership override, billing entitlement, Stripe lock, and per-learner audit; no learner/Mentor chrome | **Linear F3c spec + code win** | 2026-08-24 |
