...
| Artifact roadmap chrome | Centered page `<h1>` + inline mentor card | Topbar track name + actions | **`ArtifactShell`** topbar (`artifact-topbar`): right cluster `items-end` — single `mentor-report-link` (`h-9`, `FileText` icon slot) + track name block (no ring in topbar); **`TrailProgressRing`** (`trail-progress-ring`, ~44px, mint stroke) centered in **page intro** below subtitle when checklist items exist — `getTrailChecklistProgressPct` item-pooled; label **Progresso de estudo**; **`VerticalSpine`** — solid horizontal **`roadmap-connector-{id}`** (2px) card ↔ spine dot (`border` default, `warning` revisar, `accent-mint` selected); adaptive view (`?adaptive=1`) = subtitle + highlighted spine node only — **no** `MissionBanner` on canvas; without session → silent server fallback; `VerticalSpineSkeleton` mirrors connector layout while loading; spine `scrollIntoView` on select; selected spine dot mint glow | **Code wins** — de-cluttered topbar; trail progress + spine connectors on canvas | 2026-05-30 |
...
*Last updated: 2026-05-30 — solid spine card connectors (`roadmap-connector-{id}`) on vertical roadmap canvas*
