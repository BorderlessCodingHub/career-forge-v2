...
| IdentityGate password (CAR-106) | Not in prototype | Platform sign-in layout (not a clone) | Overlay on product routes: email+password → CF `POST /auth/signin`; BrandLockup + Back to Welcome; signup modal then Borderless `_blank`; no OAuth; no `/sign-in` route | **Grill 2026-09-11 + code win** | 2026-09-12 |
