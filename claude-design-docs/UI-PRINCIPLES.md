...
| Premium A/B HTML | CAR-41 bake-off — `/welcome/premium-a` · `/welcome/premium-b`; static rewrite; **not** Next components; not linked; not the funnel |
...
| `PlgScrollPlaque` | PLG split-flap title (`rotateX`) on the first fold; visual only (`aria-hidden`); sections keep `sr-only` H2s |
